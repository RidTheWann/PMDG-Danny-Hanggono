...
<tbody className="divide-y divide-gray-700">
  {paginatedPatients.map((patient, index) => (
    <tr key={index} className="group hover:bg-gray-700 transition-colors">
      <td className="px-2 py-2 text-sm font-medium text-gray-900 dark:text-gray-300 group-hover:text-white">{(currentPage - 1) * pageSize + index + 1}</td>
      <td className="px-2 py-2 text-sm text-gray-900 dark:text-gray-300 group-hover:text-white">{patient.tanggal}</td>
      <td className="px-2 py-3">
        <div className="truncate">
          <div className="text-xs font-medium text-gray-900 dark:text-white group-hover:text-white truncate">{patient.nama_pasien}</div>
          <div className="text-xs text-gray-700 dark:text-gray-400 group-hover:text-white truncate">{patient.kelamin === 'L' ? 'Laki-laki' : patient.kelamin === 'P' ? 'Perempuan' : patient.kelamin}</div>
        </div>
      </td>
      <td className="px-2 py-3 text-xs text-gray-900 dark:text-white group-hover:text-white truncate">{patient.no_rm}</td>
      <td className="px-2 py-3">
        <div className="flex flex-wrap gap-1 max-w-[250px]">
          {patient.actions && patient.actions.length > 0 ? patient.actions.map((action, idx) => (
            <span key={idx} className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-300 group-hover:bg-gray-600 group-hover:text-white">
              {action}
            </span>
      ...
      <td className="px-2 py-3 text-xs text-gray-900 dark:text-white group-hover:text-white truncate">{patient.lainnya || '-'}</td>
