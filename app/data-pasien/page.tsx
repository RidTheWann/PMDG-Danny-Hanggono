...
import { ArrowLeft, ArrowRight, Users, Activity, Calendar } from 'lucide-react';
...
export default function DataPasienPage(): JSX.Element {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900">
        <div className="text-center">
          ...
          {error && (
            <div
              className="mt-4 text-red-500 bg-red-100 dark:bg-red-200 px-4 py-2 rounded-lg text-center"
              role="alert"
            >
              {error}
              <button
  ...
  const totalPerempuan = patients.filter(
    (p: Patient) => p.kelamin === 'Perempuan' || p.kelamin === 'P',
  ).length;
  ...
  const rataKunjunganHarian =
    tanggalUnik.length > 0 ? Math.round(totalSemuaPasien / tanggalUnik.length) : 0;

  // Hitung rata-rata laki-laki dan perempuan per bulan
  const bulanUnik = Array.from(
    new Set(patients.map((p: Patient) => (p.tanggal || '').slice(0, 7))),
  ).filter(Boolean);
  const totalLakiPerBulan = bulanUnik.length > 0 ? Math.round(totalLaki / bulanUnik.length) : 0;
  const totalPerempuanPerBulan =
    bulanUnik.length > 0 ? Math.round(totalPerempuan / bulanUnik.length) : 0;
  ...
  patients.forEach((p) => {
    ...
    if (['laki-laki', 'laki-laki', 'laki-laki', 'l'].includes(kelamin))
      trafficData[p.tanggal].laki++;
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 transition-colors duration-500">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl mb-8 transition-colors duration-500">
          ...
          <div className="overflow-x-auto scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 scrollbar-track-gray-100 dark:scrollbar-track-gray-900 mobile-table-container">
            {loading ? (
              ...
              <>
                {/* Desktop Table View */}
                <div className="hidden sm:block min-w-[1200px] w-full align-middle">
                  <div className="overflow-hidden rounded-md">
                    <table className="w-full table-fixed">
                      <thead className="bg-gray-700">
                        <tr>
                          <th className="w-[40px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            No
                          </th>
                          <th className="w-[90px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Tanggal
                          </th>
                          <th className="w-[180px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Nama Pasien
                          </th>
                          <th className="w-[100px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            No. RM
                          </th>
                          <th className="w-[250px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Tindakan
                          </th>
                          <th className="w-[80px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Jenis
                          </th>
                          <th className="w-[150px] px-2 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Lainnya
                          </th>
                          <th className="w-[80px] px-2 py-3 text-center text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Aksi
                          </th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-700">
                        {paginatedPatients.map((patient, index) => (
                          <tr key={index} className="group hover:bg-gray-700 transition-colors">
                            <td className="px-2 py-2 text-sm font-medium text-gray-900 dark:text-gray-300 group-hover:text-white">
                              {(currentPage - 1) * pageSize + index + 1}
                            </td>
                            <td className="px-2 py-2 text-sm text-gray-900 dark:text-gray-300 group-hover:text-white">
                              {patient.tanggal}
                            </td>
                            <td className="px-2 py-3">
                              <div className="truncate">
                                <div className="text-xs font-medium text-gray-900 dark:text-white group-hover:text-white truncate">
                                  {patient.nama_pasien}
                                </div>
                                <div className="text-xs text-gray-700 dark:text-gray-400 group-hover:text-white truncate">
                                  {patient.kelamin === 'L'
                                    ? 'Laki-laki'
                                    : patient.kelamin === 'P'
                                      ? 'Perempuan'
                            ...
                            <td className="px-2 py-3 text-xs text-gray-900 dark:text-white group-hover:text-white truncate">
                              {patient.no_rm}
                            </td>
                            <td className="px-2 py-3">
                              <div className="flex flex-wrap gap-1 max-w-[250px]">
                                {patient.actions && patient.actions.length > 0 ? (
                                  patient.actions.map((action, idx) => (
                                    <span
                                      key={idx}
                                      className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-gray-300 group-hover:bg-gray-600 group-hover:text-white"
                                    >
                                      {action}
                                    </span>
                                  ))
                            ...
                            <td className="px-2 py-3">
                              <span
                                className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${patient.jenis_pasien === 'BPJS' ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 group-hover:bg-blue-800 group-hover:text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-gray-200 group-hover:bg-gray-600 group-hover:text-white'}`}
                              >
                    ...
                    {paginatedPatients.map((patient, index) => (
                      <div key={index} className="bg-gray-700 rounded-lg p-4 shadow-md">
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <span
                            className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${patient.jenis_pasien === 'BPJS' ? 'bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200' : 'bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-gray-200'}`}
                          >
                            {patient.jenis_pasien}
                          </span>
                        </div>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {patient.actions && patient.actions.length > 0 ? (
                              patient.actions.map((action, idx) => (
                                <span
                                  key={idx}
                                  className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-gray-600 text-gray-300"
                                >
                                  {action}
                                </span>
                              ))
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mb-2">Tidak ada kunjungan pada tanggal ini</p>
                <p className="text-sm text-gray-500">Pilih tanggal lain atau tambahkan data baru</p>
              </div>
            )}
          </div>
        ...
        <section className="mb-12">
          <div
            className={`mb-8 text-center transform transition-all duration-700 ${pageLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}
          >
